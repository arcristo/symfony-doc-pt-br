...
Quando sua aplicação recebe uma requisição, ela executa uma
:doc:`ação do controlador </controller>` para gerar a resposta. A configuração
de roteamento define qual ação executar para cada URL recebido. Ela também
fornece outros recursos úteis, como a geração de URLs amigáveis (por exemplo,
``/read/intro-to-symfony`` em vez de ``index.php?article_id=57``).
...
As rotas podem ser configuradas em YAML, XML, PHP ou usando annotations. Todos
os formatos oferecem os mesmos recursos e desempenho, então escolha o seu favorito.
:ref:`O Symfony recomenda annotations <best-practice-controller-annotations>`
porque é conveniente colocar a rota e o controlador no mesmo local.

Criando Rotas como Annotations
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Execute este comando uma vez na sua aplicação para adicionar o suporte a annotations:

.. code-block:: terminal

    $ composer require annotations

Além de instalar as dependências necessárias, esse comando cria o seguinte
...
Essa configuração diz ao Symfony para procurar por rotas definidas como
annotations em qualquer classe PHP armazenada no diretório ``src/Controller/``.

Suponha que você queira definir uma rota para o URL ``/blog`` na sua aplicação. Para
fazer isso, crie uma :doc:`classe controlador </controller>` como a seguinte::
...
Essa configuração define uma rota chamada ``blog_list`` que corresponde quando o
usuário solicita o URL ``/blog``. Quando a correspondência ocorre, a aplicação
executa o método ``list()`` da classe ``BlogController``.

.. note::

    Os parâmetros de um URL não são considerados ao corresponder rotas. Nesse
    exemplo, URLs como ``/blog?foo=bar`` e ``/blog?foo=bar&bar=foo`` também
    irão corresponder à rota ``blog_list``.

O nome da rota (``blog_list``) não é importante por enquanto, mas será
essencial mais tarde ao :ref:`gerar URLs <routing-generating-urls>`. Você só
deve ter em mente que cada nome de rota deve ser único na aplicação.

Criando Rotas em Arquivos YAML, XML ou PHP
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. configuration-block::
    ...
    .. code-block:: yaml
        ...
        blog_list:
            path: /blog
            # o valor de controller segue o formato 'classe_controlador::nome_do_metodo'
            controller: App\Controller\BlogController::list

            # se a ação for implementada como o método __invoke() da
            # classe controlador, você pode omitir a parte '::nome_do_metodo':
...
Correspondendo a Métodos HTTP
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Por padrão, as rotas correspondem a qualquer verbo HTTP (``GET``, ``POST``, ``PUT``, etc.)
Use a opção ``methods`` para restringir os verbos aos quais cada rota deve responder:
...
.. configuration-block::
    ...
    .. code-block:: php-annotations
        ...
        class BlogApiController extends AbstractController
        {
            ...
            {
                // ... retorna uma resposta JSON com o post
            }
...
.. tip::
    ...
    Os formulários HTML suportam apenas os métodos ``GET`` e ``POST``. Se você estiver chamando uma
    rota com um método diferente a partir de um formulário HTML, adicione um campo oculto chamado
    ``_method`` com o método a ser usado (por exemplo, ``<input type="hidden" name="_method" value="PUT"/>``).
    Se você cria seus formulários com o :doc:`Symfony Form </forms>` isso é feito
...
.. configuration-block::
    ...
    .. code-block:: php-annotations
        ...
        class DefaultController extends AbstractController
        {
             ...
             * )
             *
             * as expressões também podem incluir parâmetros de configuração:
...
``request``
    ...
    representa a requisição atual.

Nos bastidores, as expressões são compiladas em PHP puro. Por isso,
o uso da chave ``condition`` não causa nenhuma sobrecarga extra além do tempo necessário
para executar o PHP resultante.

.. caution::

    Conditions *não* são levadas em consideração ao gerar URLs (explicado
...
Passe o nome (ou parte do nome) de alguma rota para esse argumento para imprimir os
detalhes da rota:
